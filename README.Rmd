...
![CRAN Badge](http://www.r-pkg.org/badges/version/gbfs) [![Build Status](https://travis-ci.org/ds-civic-data/gbfs.svg?branch=master)](https://travis-ci.org/ds-civic-data/gbfs) <img style="float: right;" src="figures/hex.png" width="240">
...
With a few lines of `gbfs`, `dplyr`, and `ggplot2`, we can pick together a
...
above for clarity.)
* We set `output = "return"` in all of the above calls---if you leave the output
argument as default and supply a directory, `gbfs` will save the dataframes
in your local files.  
